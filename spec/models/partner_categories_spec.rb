...
describe 'Partner Categories', type: :model do
  ...
  it 'should have unique urls' do
    expect(@collection.map do |model|
             Capybara.app.url_friendly_string(model[:name])
           end.uniq.count).to eq(@collection.count)
  end
  ...
  it 'should have localization on the data' do
    ...
    schema = @data.map { |_i, x| x.select { |_k, v| v.is_a?(::Hash) }.map { |k, v| [k, v.to_h.keys] } }.flatten(1).uniq

    [:name].each do |field|
      schema.select { |key, _locales| key.to_s == field.to_s }.each do |_key, locales|
  ...
  it 'should have valid data in production' do
    ...
    @collection.each do |model|
      is_valid = begin
        model && model[:name] && model[:url_slug]
