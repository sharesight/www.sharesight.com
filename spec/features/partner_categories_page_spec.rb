...
describe 'Partner Category Pages', type: :feature do
  ...
  Capybara.app.data.locales.each do |locale|
    it "should have expected urls for locale #{locale['id']}" do
      get_partners_categories(locale, all: true).each do |category|
        subset = 0
        while category[:partners].length - subset > 0
          subset += @per_page
          ...
          expect(page).to have_head('link',
                                    args: { rel: 'canonical', href: localize_url(page_path, locale_id: locale[:id]) }, debug: :href)
          ...
          locales.each do |alternate_locale|
            if alternate_locale[:id] == default_locale_id
              expect(page).to have_head('link',
                                        args: { rel: 'alternate', href: base_url(page_path), hreflang: 'x-default' }, debug: :href)
            end
            expect(page).to have_head('link',
                                      args: { rel: 'alternate', href: localize_url(page_path, locale_id: alternate_locale[:id]), hreflang: alternate_locale[:lang].downcase }, debug: :href)
          end
          ...
          if prev
            expect(page).to have_head('link', args: { rel: 'prev', href: localize_url(prev, locale_id: locale[:id]) },
                                              debug: :href)
          end
          if category[:partners].length - subset > 0
            expect(page).to have_head('link',
                                      args: { rel: 'next', href: localize_url("#{category[:path]}/pages/#{page_num + 1}/", locale_id: locale[:id]) }, debug: :href)
          end
        end
      end
    end
    ...
    it "should have the expected elements for locale #{locale['id']}" do
      ...
      categories.each do |category|
        ...
        expect(page).to have_selector(:css, 'a.btn', text: 'Learn More',
                                                     count: category[:partners].length > @per_page ? @per_page : category[:partners].length)

        if category[:id] != 'all'
          expect(page).to have_css('.partner-card__category',
                                   text: category[:title].singularize)
        end
        ...
        if category[:partners].length > @per_page
          expect(page).to have_css("a[href='#{localize_url("#{category[:path]}/pages/2/", locale_id: locale[:id])}']",
                                   text: 'Next Page')
        end
