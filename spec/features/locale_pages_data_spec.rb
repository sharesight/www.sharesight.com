...
describe 'Locale Pages', type: :feature do
  ...
  def expect_canonical_urls(page, page_data, locale)
    ...
    expect(page).to have_head('link',
                              args: { rel: 'canonical', href: localize_url(page_data[:page], locale_id: locale[:id]) }, debug: :href)

    Capybara.app.page_alternative_locales(page_data[:page]).each do |alternate_locale|
      if alternate_locale[:id] == default_locale_id
        expect(page).to have_head('link',
                                  args: { rel: 'alternate', href: localize_url(page_data[:page], locale_id: default_locale_id), hreflang: 'x-default' }, debug: :href)
      end
      expect(page).to have_head('link',
                                args: { rel: 'alternate', href: localize_url(page_data[:page], locale_id: alternate_locale[:id]), hreflang: alternate_locale[:lang]&.downcase }, debug: :href)
    end
  end

  def expect_taglines(page, page_data, locale)
    %w[tagline footer_tagline].each do |tag|
