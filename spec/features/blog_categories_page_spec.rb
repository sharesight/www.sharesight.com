...
describe 'Blog Category Pages', type: :feature do
  ...
  it 'should have expected urls' do
    @categories.each do |category|
      ...
      while category.posts.length - subset > 0
        subset += @per_page
  ...
  it 'should have the expected elements' do
    @categories.each do |category|
      ...
      expect(page).to have_css('h1.heading_page',
                               text: is_all_category || is_sharesight20_category ? category.title : "Topic: #{category.title}")

      # Expect this to have posts.
      expect(page).to have_selector(:css, 'a.btn', text: 'Read Full Post',
                                                   count: category.posts.length > @per_page ? @per_page : category.posts.length)
      ...
      if is_sharesight20_category
        ...
        @categories.each do |cat|
          unless cat.name == 'All'
            expect(page).to have_css("nav[role='navigation'] li a[href='#{cat.url}']",
                                     text: "#{cat.title} (#{cat.posts.length})")
          end
        end
      end

      # Check for the next page.
      if category.posts.length > @per_page
        expect(page).to have_css("a[href='#{base_url("#{category[:path]}/pages/2/")}']",
                                 text: 'Older posts')
      end
