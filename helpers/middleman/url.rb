...
module MiddlemanUrlHelpers
  ...
  def canonical_url
    absolute_url(get_current_page_url)
  end
  ...
  def url_friendly_string(str)
    BasicHelper.url_friendly_string(str)
  end
  ...
  def base_url(append = '/', base_url: config[:base_url])
    localize_url(append, base_url: base_url, locale_id: default_locale_id)
  end

  def unlocalized_url(append = '/', base_url: config[:base_url])
    base_url(append, base_url: base_url)
  end
  ...
  def localize_path(append = '/', locale_id: current_locale_id)
    localize_url(append, locale_id: locale_id, base_url: '')
  end
  ...
  def localize_url(append = '/', locale_id: current_locale_id, base_url: config[:base_url])
    ...
    if (base_url == config[:base_url] || base_url == '') && is_valid_page?(page) && !is_valid_locale_id_for_page?(page,
      ...
      unless is_valid_locale_id_for_page?(
        page, locale_id
      )
        raise ArgumentError,
              "Attempted to localize a path to an unsupported locale: #{locale_id} for page: #{page}."
      end
    end

    unless is_valid_locale_id?(locale_id)
      raise ArgumentError,
            "Attempted to localize a path to an invalid locale: #{locale_id}."
    end
  ...
  def page_from_path(path)
    ...
    while i < split.length
      return split[i] unless split[i].blank?

      i += 1
    end

    nil # (shouldn't actually reach this point – it would have to be an array of blanks, which shouldn't happen via split)
  end

  def wrap_path_in_slashes(path)
    # ensure append is wrapped in singular forward slashes
    path = "/#{path}"
    path = "#{path}/" unless ['#', '?', '&', '.'].any? do |needle|
                               path&.split('/')&.last&.include?(needle)
                             end
    path.squeeze('/') # Adjacent multiple slashes should always resolve to a single slash as per HTTP RFC.
  end
  ...
  def strip_locale_from_path(path)
    ...
    wrap_path_in_slashes(split.join('/'))
  end
